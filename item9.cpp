...
// But also alias can be used to create alias temples which is not possible with typedef
template <typename T>
using HashVector = std::unordered_map<T, std::vector<T>>;
...
struct Foo {
    ...
    typename vectorTypeStruct<T>::type v2;  // with the previos syntax one needed to specify that vectorTypeStruct<T>::type is a type
};
...
int main(int argc, char *argv[]) {
    ...
    std::unique_ptr<std::unordered_map<int, std::string>> um_ptr_artritis =
        std::make_unique<std::unordered_map<int, std::string>>(std::unordered_map<int, std::string>());  // to be fair I could have use an auto, but this proves a point
