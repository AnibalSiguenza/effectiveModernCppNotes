/** This items talks about how refernce collapsing works. In short it is an and operation for rvalue reference:
 * there 2 types of reference so the function is either expecting a lvalue reference or an rvalue reference,
 * and one can parse either a rvalue or an lvalue wich give us 3 possibilities
 * lvalue -> f(&) gives a lvalue reference
 * lvalue -> f(&&) gives a lvalue reference
 * rvalue -> f(const &) gives a lvalue reference
 * rvalue -> f(&&) gives a rvalue reference
 * So only if the reference as well as the value ar r the result will be an r value ref
 */
#include <bits/stdc++.h>

template <class T>
void lReferenceFunction(const T &t) {
}
template <class T>
void rReferenceFunction(T &&t) {
}
