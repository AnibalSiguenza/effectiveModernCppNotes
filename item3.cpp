...
//This item explains the decltype function which returns the type of the variable passed as argument
...
//Usefull examples:
// This does not return access to the container because the returned variable loses the reference properties
...
decltype(auto) foo() {
    ...
    return (a);  //this returns a reference to a local variale which can lead to mistakes
}
...
int main(int argc, char* argv[]) {
    ...
    // All this cases return exactly what is expected which is the exact match of types
    ...
    // int& wrongElem = wrongAutchAndAccess(v1, 2); //compilation error because the return type is only int
    // wrongElem = 13;
