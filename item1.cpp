...
template <typename T> void fvalue(T t) {
  // t(1, 2);
}

template <typename T> void fref(T &t) {
  // t(1, 2);
}
...
int main(int argc, char *argv[]) {
  ...
  fref(a_ref);            // simply gets fref(int&)
  ...
  fref(a_const_ptr_int);  // passes the reference to a pointer with the const
                          // fref(const int *&t)
  fref(a_2const_ptr_int); // passes the reference to a pointer with both consts
                          // fref(const int const *&t)
