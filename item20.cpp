// This items talks about weak_ptr and its uses. One interesting remark is that
// it is not an stand alone pointer. it is an extension of the share_ptr
#include <bits/stdc++.h>

#include "TestClass.hpp"

void testWeakPtr(std::weak_ptr<TestClass> &wp) {
  // To use it one needs to use the lock operation, it cannot be directly
  // derreferenced
  auto tempSharedPtr = wp.lock();

  // if the object to which the weak pointer was deleted it will point to
  // nullptr
...
int main(int argc, char *argv[]) {
            ...
            << std::endl;
  {
    ...
    std::cout << "before scope 2.1" << std::endl;
    {
      std::shared_ptr<TestClass> sptr2(new TestClass("object2"));
      weakPtr2 = sptr2;
      sptr3 = weakPtr2.lock(); // the shared pointer which results from the lock
                               // function does have ownership to the object
    }
